  ...
  module Chat
    ...
    module Closure
      ...
      class << self
        ...
        # нет, и следующее его сообщение уйдёт в никуда. Со звуком: состав
        # сбрасывают вечером, и это письмо человеку лучше прочитать сразу
        # (ночь в чужом часовом поясе доберёт Telegram::QuietHours). Двумя
        # проходами и с оговоркой на каждого: сброс состава и удаление игры уже
        # случились, второго захода по этим людям не будет — так что одна
        # упавшая отправка не должна ни оставить остальных в мёртвом чате, ни
        # съесть их письма.

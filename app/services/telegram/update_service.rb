  ...
  class UpdateService
    ...
    def self.process(update)
      ...
      if (cq = update["callback_query"])
        ...
        # manage callback (show per-participant remove buttons)
        if data =~ /\Amanage:(\d+)\z/
          game_id = $1.to_i
          owner = User.find_by(telegram_chat_id: chat_id.to_s)
          ...
          if game && owner && game.user_id == owner.id
            ...
            if participants.empty?
              poller.send_api("answerCallbackQuery", { callback_query_id: cq["id"], text: "No other participants to manage", show_alert: false })
              poller.send_api("sendMessage", { chat_id: chat_id, text: "No other participants to manage for game ##{game_id}." })
            else
              rows = participants.map { |p| [{ text: "Remove #{p.name || p.email}", callback_data: "remove:#{game.id}:#{p.id}" }] }
              poller.send_api("answerCallbackQuery", { callback_query_id: cq["id"], text: "Manage players", show_alert: false })
              poller.send_api("sendMessage", { chat_id: chat_id, text: "Participants for game ##{game_id}:", reply_markup: { inline_keyboard: rows } })
            end
          else
            poller.send_api("answerCallbackQuery", { callback_query_id: cq["id"], text: "Only the game owner can manage players", show_alert: false })
          end
          return
        end

        # remove callback (owner removes participant)
          ...
          if game && owner && game.user_id == owner.id
            ...
            if participation
              ...
              begin
                ...
                Rails.logger.error "[BOT] Poller.remove error: #{e.class} #{e.message}"

  ...
  class CallbackHandler
    def self.handle(callback_query)
      ...
      when /\Atg_fill:(\d+)\z/
        ...
        Rails.cache.write(
          key,
          { "game_id" => game_id, "step" => "ask_singles_hours", "created_at" => Time.current },
          expires_in: 2.hours
        )

        Api.answer_callback(callback_id)
        Api.send_force_reply(chat_id, "Please reply with singles hours (example: 1.5) or type 'skip'.")
      ...
      when /\Atg_not_happened:(\d+)\z/
        ...
        Api.answer_callback(callback_id, "Unknown action.", show_alert: true)
      end
    rescue => e

class TrainingBlock
  ...
  class Diagram
    ...
    # и с константами в court_diagram_controller.js: 100 единиц в ширину,
    # 200 в длину — это пропорции корта вместе с забегами.
    WIDTH = 100.0
    HEIGHT = 200.0
    ...
    class << self
      def normalize(raw)
        ...
        return EMPTY.deep_dup if frames.all? { |frame| blank_frame?(frame) }
      ...
      def normalize_frame(raw)
        ...
        {
          "title" => frame["title"].to_s.strip.first(MAX_TITLE),
      ...
      def blank_frame?(frame)
        frame["title"].blank? && frame["items"].empty? && frame["arrows"].empty?
      end
